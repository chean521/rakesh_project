...
$Query = "Select QtPrefix from company where UserId=".$Company_Id.";";
$Query .= "SELECT QuotationNo FROM quotation WHERE CompanyId=".$Company_Id." order by (QuotationNo) DESC;";
        ...
        if($res = $Conn->store_result()){
            ...
            if($query_counter == 0){
                while($row = $res->fetch_assoc()){
                    $InvPref = $row["QtPrefix"];
                }
            }
            else{
                while($row = $res->fetch_assoc()){
                    array_push($Inv_No, $row["QuotationNo"]);
                }
            }
...
echo json_encode(array("NewQt"=>$NewInvoiceNo));
