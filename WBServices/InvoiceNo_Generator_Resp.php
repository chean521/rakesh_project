...
$Query = "Select InvPrefix from company where UserId=".$Company_Id.";";
$Query .= "SELECT InvoiceNo FROM taxinvoices WHERE CompanyId=".$Company_Id." order by (InvoiceNo) DESC;";
        ...
        if($res = $Conn->store_result()){
            ...
            if($query_counter == 0){
                while($row = $res->fetch_assoc()){
                    $InvPref = $row["InvPrefix"];
                }
            }
            else{
                while($row = $res->fetch_assoc()){
                    array_push($Inv_No, $row["InvoiceNo"]);
                }
            }
...
echo json_encode(array("NewTaxInv"=>$NewInvoiceNo));
